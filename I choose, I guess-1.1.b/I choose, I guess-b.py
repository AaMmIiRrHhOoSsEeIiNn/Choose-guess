...
n_data = 3  # n_data: Number of MAX/MIN geusses (n_data for MAX and n_data for MIN) (DETERMINE THIS)
...
for  data in range(n_data): # Total data (in this loop we add step range(end_range_o) and and step repeatirion(base_repaet) to last ones;
    ...
    base_repeat += 5   # In each n_data we add base_repeat more to repeats variables. (DETERMINE THIS)
    end_range_o += 100  # For each n_data, range increase as much as this.(DETERMINE THIS)
